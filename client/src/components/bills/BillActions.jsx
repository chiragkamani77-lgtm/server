 ...
 * Handles approve, reject, and mark as paid actions for bills
 * Props:
 * - bill: Bill object to act on
...
export function BillActions({ bill, isOpen, onClose, onSuccess, action = 'approve' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ submit: error.response?.data?.message || 'Failed to process bill' })
    } finally {
      setSubmitting(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-lg bg-gray-50 p-3 space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{bill?.invoiceNumber || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Base Amount:</span>
              <span className="font-medium">₹{bill?.baseAmount?.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">GST ({bill?.gstRate}%):</span>
              <span className="font-medium text-orange-600">₹{bill?.gstAmount?.toFixed(2)}</span>
            </div>
